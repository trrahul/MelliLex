...
impl OpenAIService {
    ...
    pub(crate) async fn fetch_models_internal(api_key: &str) -> Result<Vec<AiModel>> {
        ...
        // Sort by priority first, then by ID length so the canonical (shortest) ID
        // for each family comes first (e.g. "o3" before "o3-2025-04-16").
        models.sort_by_key(|m| (ModelInfoService::get_model_priority(&m.id), m.id.len()));

        // Deduplicate by display name — keep only the first (canonical) entry per name.
        let mut seen_names = std::collections::HashSet::new();
        models.retain(|m| seen_names.insert(m.name.clone()));
