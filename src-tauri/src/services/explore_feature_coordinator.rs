...
impl<'a> ExploreFeatureCoordinator<'a> {
    ...
    pub async fn generate_practice_exercises(
        ...
        info!(
            "Generating practice exercises (count: {}, force: {}) for word: {}",
            count, force, word
        );

        if !force && count == PRACTICE_CACHE_BATCH {
    ...
    pub async fn generate_mistakes(
        ...
        self.mistakes_generator
            .generate(word, dictionary_service, force)
            .await
    }
}
...
mod tests {
    ...
    async fn practice_pipeline_caches_after_first_generation() {
        ...
        let first = coordinator
            .generate_practice_exercises("spark", PRACTICE_CACHE_BATCH, false)
            .await
    ...
    async fn mistakes_use_cached_payload_before_generation() {
        ...
        let result = coordinator
            .generate_mistakes("spark", &dictionary_service, false)
            .await
